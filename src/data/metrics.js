...
export const METRIC_MODULES = [
  ...
  },
  {
    ...
    metrics: [
      ...
      {
        name: 'Mastercard 指标',
        formula: '拒付率 = 当月拒付笔数 ÷ 上月总结算笔数 × 100%；欺诈率 = 当月欺诈拒付笔数 ÷ 上月总结算笔数 × 100%；当月欺诈金额 = 当月欺诈型拒付的争议金额（Σ 欺诈拒付笔数 × 单笔金额）',
        desc: '错月口径：分子取当前自然月（Month T），分母取上一个自然月（Month T-1）的总结算交易笔数；当月欺诈金额特指欺诈型拒付（Fraud Chargeback）对应的争议金额，不含非欺诈争议。点击指标数字可查看笔数统计与结算流程。',
        source: '卡组织拒付通知 + 结算数据',
        sample: '样本实测（当月 vs 上月）：47 ÷ 15,200 = 0.31%；欺诈 18 ÷ 15,200 = 0.12%；当月欺诈型拒付金额 $4,286',
      },
      {
        name: 'Klarna 指标',
