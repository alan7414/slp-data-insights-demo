...
export const METRIC_MODULES = [
  ...
  },
  {
    ...
    metrics: [
      ...
      {
        name: '拒付率指标',
        formula: '拒付率 = 当月拒付笔数 ÷ 上月总结算笔数 × 100%；欺诈率 = 当月欺诈拒付笔数 ÷ 上月总结算笔数 × 100%；当月欺诈拒付金额 = 当月欺诈型拒付（Fraud Chargeback）的争议金额',
        desc: '错月口径：分子取最近完整月（Month T），分母取上一个完整月（Month T-1）的总结算交易笔数；统一口径统计，不区分 Visa / Mastercard 卡组织品牌。',
        source: '卡组织拒付通知 + 结算数据',
        sample: '样本实测（最近完整月 vs 上月）：65 ÷ 21,678 = 0.300%；欺诈 28 ÷ 21,678 = 0.129%；当月欺诈拒付金额 USD 2,843',
      },
      {
        name: '预拒付工具效果',
        formula: '预拒付拦截笔数 = 当月预拒付 accept 笔数 + ehoca-refund 笔数；拦截前拒付率 =（当月拒付笔数 + 拦截笔数）÷ 上月总结算笔数；拒付率下降 = 拦截笔数 ÷（当月拒付笔数 + 拦截笔数）× 100%',
        desc: '预拒付工具在正式拒付发生前向持卡人先行退款，避免形成正式拒付记录，直接减少拒付率分子（当月拒付笔数），从而降低拒付率指标；ehoca-refund 为预拒付通道的先行退款类型。',
        source: '预拒付工具 / accept / ehoca-refund 记录 + 拒付数据',
        sample: '样本实测：accept 8 笔 + ehoca-refund 5 笔 = 13 笔，拒付率由 0.364% 降至 0.300%（下降 16.7%）',
      },
    ],
  },
];
