...
export const METRIC_MODULES = [
  ...
  },
  {
    ...
    metrics: [
      {
        name: '拒付总览（按状态）',
        formula: '拒付笔数 = 新产生的拒付；待回应 + 已回应 + 已过期 = 全部拒付；已回应 = WON + 失败；已过期 = 逾期未回应自动败诉',
        desc: '按筛选时间范围统计拒付生命周期状态；WON = 抗辩胜诉（资金退回），失败 = 抗辩败诉，已过期 = 未在回应期内回应自动败诉；支持按支付方式筛选（全部 / 卡支付方式 / Klarna / Affirm / Cash App），拒付按支付方式确定性拆分（卡 ~45%、Klarna ~38%、Affirm ~11%、Cash App ~7%，各档之和 = 全部拒付笔数）。',
        source: 'Dispute Notification / 拒付状态 / 抗辩结果 / Payment Method',
        sample: '样本实测（近 30 天全部账户）：拒付 492 笔，待回应 138、已回应 305（WON 143 / 失败 162）、已过期 49',
      },
      {
        name: '欺诈和拒付指标',
      ...
      {
        name: '拒付理由统计',
        formula: '按拒付原因（Reason Code 语义归并）聚合笔数与占比：欺诈拒付 / 未授权交易 / 余额不足 / 未收到商品服务 / 重复扣款 / 其它；按笔数由高到低排列',
        desc: '拒付理由按持卡人主张的拒付原因归并统计，随支付方式筛选联动；用于定位拒付成因（如欺诈占比高需加强风控、余额不足需优化卡片预校验）。',
        source: 'Dispute Reason Code / 拒付原因',
        sample: '样本实测（近 30 天全部账户，由高到低）：欺诈拒付 138、未授权交易 118、余额不足 89、未收到商品服务 69、重复扣款 49、其它 30',
      },
    ],
  },
];
