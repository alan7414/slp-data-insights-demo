...
export function linkFlow(agg) {
  ...
  const nodes = [
    ...
    { name: '继续交易', value: continueTx, level: 1, color: '#2563eb' },
    { name: '用户取消 / 超时', value: userCancel, level: 1, color: '#94a3b8', reason: '超时未支付、主动取消等用户行为' },
    { name: '风控未拦截', value: noRisk, level: 2, color: '#2563eb' },
    { name: '风控拦截', value: riskBlock, level: 2, color: '#f59e0b', reason: '命中风控规则（Fraud Screen Declined）' },
    ...
    { name: '3DS 未通过', value: t3Fail, level: 4, color: '#dc2626', reason: '3DS 认证失败、3DS 超时' },
    { name: '支付成功（3DS 链路）', value: t3PassSucc, level: 5, color: '#059669' },
    { name: '支付失败（3DS 链路）', value: t3PassFail, level: 5, color: '#dc2626', reason: '发卡行疑似欺诈、余额不足、卡已过期等' },
    { name: '支付成功（非 3DS 链路）', value: no3dsSucc, level: 5, color: '#059669' },
    { name: '支付失败（非 3DS 链路）', value: no3dsFail, level: 5, color: '#dc2626', reason: '发卡行疑似欺诈、余额不足、卡已过期等' },
  ];
  const links = [
    { source: '全部交易', target: '继续交易', value: continueTx },
    { source: '全部交易', target: '用户取消 / 超时', value: userCancel },
    { source: '继续交易', target: '风控未拦截', value: noRisk },
    { source: '继续交易', target: '风控拦截', value: riskBlock },
    { source: '风控未拦截', target: '需要 3DS', value: need3ds },
    { source: '风控未拦截', target: '不需要 3DS', value: no3ds },
