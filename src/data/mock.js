...
export function linkFlow(agg, opts) {
  ...
  const nodes = [
    ...
    { name: '需要 3DS', value: need3ds, level: 3, color: '#8b5cf6' },
    { name: '不需要 3DS', value: no3ds, level: 3, color: '#2563eb' },
    { name: '3DS 通过', value: t3Pass, level: 4, color: '#8b5cf6' },
    { name: '3DS 未通过', value: t3Fail, level: 4, color: '#dc2626',
      codes: [
        { code: 'SLP 4452', desc: '3DS 认证失败（authentication_declined）', count: Math.round(t3Fail * 0.70) },
        { code: 'T3_TIMEOUT', desc: '3DS 认证超时未完成', count: t3Fail - Math.round(t3Fail * 0.70) },
      ] },
    { name: '支付成功（3DS 链路）', value: t3PassSucc, level: 5, color: '#059669' },
    { name: '支付失败（3DS 链路）', value: t3PassFail, level: 5, color: '#dc2626',
      codes: [
        { code: 'SLP 4463 / 59', desc: '发卡行疑似欺诈', count: Math.round(t3PassFail * 0.40) },
        { code: '51', desc: '余额不足', count: Math.round(t3PassFail * 0.25) },
        { code: '54', desc: '卡已过期', count: Math.round(t3PassFail * 0.20) },
        { code: 'OTHER_DECLINE', desc: '银行拒付 - 其它原因', count: t3PassFail - Math.round(t3PassFail * 0.85) },
      ] },
    { name: '支付成功（非 3DS 链路）', value: no3dsSucc, level: 5, color: '#059669' },
    { name: '支付失败（非 3DS 链路）', value: no3dsFail, level: 5, color: '#dc2626',
      codes: [
        { code: 'SLP 4463 / 59', desc: '发卡行疑似欺诈', count: Math.round(no3dsFail * 0.38) },
        { code: '51', desc: '余额不足', count: Math.round(no3dsFail * 0.27) },
  ...
  const links = [
    ...
    { source: '成功发起交易', target: '风控通过', value: noRisk },
    { source: '成功发起交易', target: '风控拦截', value: riskBlock },
    { source: '风控通过', target: '需要 3DS', value: need3ds },
    { source: '风控通过', target: '不需要 3DS', value: no3ds },
    { source: '需要 3DS', target: '3DS 通过', value: t3Pass },
    { source: '需要 3DS', target: '3DS 未通过', value: t3Fail },
    { source: '3DS 通过', target: '支付成功（3DS 链路）', value: t3PassSucc },
