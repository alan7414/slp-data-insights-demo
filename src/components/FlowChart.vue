...
/* 拖动 + 点击（移动距离 < 5px 视为点击，终止节点弹错误码明细） */
const drag = ref(null);
const down = ref(null);
...
<template>
  <div class="flow-wrap" :style="{ height: H + 'px' }">
    ...
    <div v-for="n in flow.nodes" :key="n.name" class="flow-node" :class="nodeCls(n)"
      ...
      <div v-if="n.codes" class="fn-tag">👆 点击查看错误码明细</div>
    </div>
...
.fn-tag { margin-top: 7px; font-size: 10.5px; color: var(--accent); font-weight: 600; white-space: nowrap; }
